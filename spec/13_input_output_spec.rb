...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match { |num| num <= 9 and num >= 0 }
      end

      # remove the 'x' before running this test
      it 'is a number between 0 and 9' do
        sol = game.solution
        expect(sol).to be_between_zero_and_nine
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid = 'a'
        valid = '7'
        allow(game_loop).to receive(:player_input).and_return(invalid, invalid, valid)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_end_in_3_turns) { described_class.new 2, '2', 3 }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_end_in_3_turns.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_end) { described_class.new 1, '1', 5 }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_end.final_message }.to output(phrase).to_stdout
      end
    end
  end
end
