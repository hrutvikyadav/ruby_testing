...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:instance) { described_class.new 'rum' }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(instance.type).to eq('rum')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(instance.ounces).to eql(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:i) { described_class.new 'milk' }
      # remove the 'x' before running this test
      it 'is full' do
        expect(i).to be_full
      end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:i) { described_class.new 'waoa', 9 }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(i).not_to be_full
      end
    end
  end
end
